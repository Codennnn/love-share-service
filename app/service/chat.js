...
class ChatService extends Service {
  ...
  async addContactByAdmin(_id, contact_id) {
    const count = await this.ctx.model.Admin.countDocuments({
      _id, contacts: { $in: [contact_id] },
    })
  ...
  deleteContactByAdmin(_id, contact_id) {
    return this.ctx.model.Admin
      .updateOne(
        { _id },
        {
  ...
  addChatDataByAdmin({ sender, receiver }) {
    Promise.all([
      this.ctx.model.Admin
        .updateOne(
          { _id: sender.client, 'chats.contact_id': sender.target },
          {
            $addToSet: {
